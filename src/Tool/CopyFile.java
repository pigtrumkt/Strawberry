...
public class CopyFile extends JPanel {
	...
	JButton delTrungBtn;
	JButton openFolderIn;
	JButton openFolderOut;
	...
	public CopyFile() {
		...
		folderInputTxt.setBounds(111, 8, 840, 25);
		...
		SuggestionDropDownDecorator.decorate(folderInputTxt,
				new TextComponentSuggestionClient(Main.historyFieldInput::getSuggestions));
		add(folderInputTxt);

		folderOutputTxt = new JTextField();
		folderOutputTxt.setName("folderOutputTxt");
		folderOutputTxt.setBounds(111, 44, 840, 25);
		...
		SuggestionDropDownDecorator.decorate(folderOutputTxt,
				new TextComponentSuggestionClient(Main.historyFieldInput::getSuggestions));
		add(folderOutputTxt);
		...
		jScrollPane = new JScrollPane(fileAreaTxt, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		jScrollPane.setBounds(25, 110, 806, 565);
		add(jScrollPane);
		fileAreaTxt.addMouseListener(new MouseListener() {
		...
		openFolderIn = new JButton("...");
		openFolderIn.setBounds(955, 8, 25, 25);
		openFolderIn.setMargin(new Insets(1, 1, 1, 1));
		add(openFolderIn);
		openFolderIn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				openWindowExplorer(folderInputTxt.getText().trim());
			}
		});

		openFolderOut = new JButton("...");
		openFolderOut.setMargin(new Insets(1, 1, 1, 1));
		openFolderOut.setBounds(955, 44, 25, 25);
		add(openFolderOut);
		openFolderOut.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				openWindowExplorer(folderOutputTxt.getText().trim());
			}
		});

	}

	public void openWindowExplorer(String path) {
		try {
	...
	public void addHighlight(String[] fileTxt, int lineIdx, Color color) {
		...
		try {
			fileAreaTxt.getHighlighter().addHighlight(idxStart, idxStart + fileTxt[lineIdx].length(),
		...
		try {
			if (checkStatus()) {
				if (clearOutFol.isSelected()) {
					...
					int result = JOptionPane.showOptionDialog(this, "Xóa toàn bộ thư mục output?", "Cảnh báo",
